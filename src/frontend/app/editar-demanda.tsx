...
import { ApiError, getDemandById, updateDemand } from "@/lib/api";
...
// ─── Tela Principal ─────────────────────────────────────────────────────────
export default function EditarDemandaScreen() {
  ...
  // Carrega dados da demanda ao montar
  useEffect(() => {
    const loadDemand = async () => {
      if (!token || !id) return;
      setIsFetching(true);
      try {
        const demand = await getDemandById(token, id);
        
        // Função para formatar data ISO para DD/MM/YYYY
        const formatDateToInput = (isoDate: string | null): string => {
          if (!isoDate) return "";
        ...
        setForm({
          ...
          latitude_origem: demand.lat_origem ?? null,
          longitude_origem: demand.lon_origem ?? null,
          endereco_destino: demand.endereco_destino || "",
          latitude_destino: demand.lat_destino ?? null,
          longitude_destino: demand.lon_destino ?? null,
          valor_proposto: demand.valor_proposto?.toString() || "",
          peso_carga_kg: demand.peso_carga_kg?.toString() || "",
          status: demand.status || "aberta",
          data_coleta: formatDateToInput(demand.data_coleta),
        });
      } catch (error) {
  ...
  const handleSubmit = async () => {
    console.info("[Demanda] Iniciando atualização");
    ...
    setIsLoading(true);
    try {
      console.info("[Demanda] Enviando atualização");
      await updateDemand(token, id, payload);
      console.info("[Demanda] Atualização concluída");

      Alert.alert("Sucesso!", "Demanda atualizada com sucesso.", [
        { text: "OK", onPress: () => router.back() },
      ]);
    } catch (e) {
      console.error("[Demanda] Erro ao atualizar", e);
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Carregando demanda...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={styles.title}>Atualizar Demanda</Text>
            <Text style={styles.subtitle}>Edite os dados da carga</Text>
          </View>
        ...
        <View style={styles.formContainer}>
              ...
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 8 }}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    value={form.latitude_origem?.toString() || ""}
                ...
                <View style={{ flex: 1, marginLeft: 8 }}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    value={form.longitude_origem?.toString() || ""}
              ...
              <View style={styles.row}>
                <View style={{ flex: 1, marginRight: 8 }}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    value={form.latitude_destino?.toString() || ""}
                ...
                <View style={{ flex: 1, marginLeft: 8 }}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    value={form.longitude_destino?.toString() || ""}
          ...
          >
            {isLoading ? (
              ...
              <Text style={styles.buttonText}>Atualizar Demanda</Text>
            )}
          </TouchableOpacity>
        </View>
      </ScrollView>
